...
__device__ uchar4 pixelAt(unsigned char *input, int x, int y, int width,int height)
{
    int start = getindexForPixelAt(x, y, width, height);
    uchar4 pixel = {input[start], input[start+1], input[start+2], input[start+3]};;

    return pixel;
}

__global__ void global_filter(unsigned char* input,uchar4* output,int width, int height)
{

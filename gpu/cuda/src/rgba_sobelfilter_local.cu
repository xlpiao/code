...
__device__ uchar4 pixelAt(uchar4* localBlock, int x, int y, int width,
                          int height) {
...
__global__ void local_filter(unsigned char* input, uchar4* output, int width,
                             int height) {
  ...
  if (tx - 1 == 0 && ty - 1 == 0) {
    localBlock[tx - 1 + (ty - 1) * w].x =
        input[BPP * (x - 1 + (y - 1) * width)];
    localBlock[tx - 1 + (ty - 1) * w].y =
        input[BPP * (x - 1 + (y - 1) * width) + 1];
    localBlock[tx - 1 + (ty - 1) * w].z =
        input[BPP * (x - 1 + (y - 1) * width) + 2];
    localBlock[tx - 1 + (ty - 1) * w].w =
        input[BPP * (x - 1 + (y - 1) * width) + 3];
  }

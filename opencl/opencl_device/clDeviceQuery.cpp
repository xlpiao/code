    ...
    if (ciErrNum != CL_SUCCESS) {
        ...
        if (num_platforms == 0) {
            ...
            for(cl_uint i = 0; i < num_platforms; ++i) {
                ciErrNum = clGetPlatformInfo (clPlatformIDs[i], CL_PLATFORM_NAME, sizeof(cBuffer), &cBuffer, NULL);
                if(ciErrNum == CL_SUCCESS) {
                    ...
                    ciErrNum = clGetPlatformInfo (clSelectedPlatformID, CL_PLATFORM_EXTENSIONS, sizeof(cBuffer), cBuffer, NULL);
                    if (ciErrNum == CL_SUCCESS) {
